...
function ResultCard({
  ...
  const probability = Math.max(
    0,
    Math.min(
      result.attrition_probability > 1
  ...
  return (
    <aside
      className="sticky top-[92px] rounded-2xl border border-border bg-card p-6 shadow-md md:p-7"
    ...
    >
      ...
      <div className="mt-8 flex items-start justify-between gap-4">
        ...
        <span
          className={`rounded-full border px-3 py-1.5 text-xs font-bold ${riskClass}`}
      ...
      <div className="mt-9 border-t border-border pt-6">
        <div className="flex items-end justify-between">
          ...
          <span
            ...
            data-testid="text-attrition-probability"
          >
      ...
      <div className="mt-7 flex items-start gap-3 rounded-xl bg-muted/60 p-4 text-xs leading-5 text-muted-foreground">
        <CheckCircle2
          size={15}
        ...
        <span>
          ...
          . Treat this as a prompt for context,
          not a decision.
        </span>
      </div>
    </aside>
...
export default function Predict() {
  const [form, setForm] =
    useState<EmployeeInput>(initialEmployee);
  ...
  const handleChange = (
    ...
    const key =
      target.name as keyof EmployeeInput;
    ...
    <div className="space-y-9">
      ...
      <header className="flex flex-col justify-between gap-5 md:flex-row md:items-end">
        <div className="animate-rise-in">
          ...
          <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
            Enter the employee profile to explore an
            attrition-risk signal. The form contains the
            27 original input features used by the
            prediction API. Five engineered features are
      ...
      <div className="grid gap-7 xl:grid-cols-[minmax(0,1fr)_340px] xl:items-start">
        ...
        <form
          ...
          className="space-y-5"
          data-testid="form-predict-employee"
        >
          ...
          <div className="flex items-center gap-3 rounded-2xl border border-border bg-card px-5 py-4 shadow-sm">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                27 employee attributes are sent to the
                prediction API.
              </p>
            </div>
                ...
                <div>
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">
                    {section.title === 'Work context'
                      ? 'The environment around this employee.'
                      : section.title === 'Employee profile'
                        ? 'Basic demographic and education context.'
                        : section.title === 'Experience & growth'
                          ? 'Tenure and progression signals.'
                          : 'Self-reported and observed workplace signals.'}
                  </p>
                </div>
              </div>
          ...
          {mutation.isError && (
            <div
              className="flex items-start gap-3 rounded-2xl border border-destructive/25 bg-destructive/5 p-4 text-sm"
              data-testid="status-prediction-error"
            >
              <AlertTriangle
                size={17}
              ...
              <div>
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  Check the profile values and make sure
                  the backend is running with the trained
          ...
          <div className="flex flex-col-reverse justify-between gap-3 border-t border-border pt-5 sm:flex-row sm:items-center">
            ...
            <button
              type="button"
              onClick={handleReset}
              className="inline-flex items-center justify-center gap-2 rounded-xl px-4 py-3 text-sm font-semibold text-muted-foreground transition hover:bg-muted hover:text-foreground"
            ...
            <button
              ...
              className="inline-flex items-center justify-center gap-2 rounded-xl bg-primary px-5 py-3 text-sm font-bold text-primary-foreground transition hover:-translate-y-0.5 hover:shadow-md disabled:cursor-wait disabled:opacity-60"
        ...
        <div>
          ...
          {result ? (
            ...
            <div
              className="sticky top-[92px] rounded-2xl border border-dashed border-border bg-card/60 p-6 md:p-7"
              data-testid="status-prediction-empty"
            >
              ...
              <p className="mt-3 text-sm leading-6 text-muted-foreground">
                Complete the employee profile on the
                left, then run the saved model. The
                response includes the prediction,
                probability, risk level, and actual model
                used.
              </p>
